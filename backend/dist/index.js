...
async function getSmartDataContext(question) {
...
function generateAnalyticsContext(trips, users, tripStats) {
...
function analyzeTimePatterns(trips) {
    const hourCounts = {};
...
function analyzeLocationPatterns(trips) {
    const pickupCounts = {};
    const dropoffCounts = {};
    trips.forEach(trip => {
...
function analyzeUserDemographics(users) {
    const ageCounts = {};
    ...
    users.forEach(user => {
        if (user.age && user.age > 0) {
            ...
            validAgeCount++;
            if (user.age < 25)
                ageCounts['18-24'] = (ageCounts['18-24'] || 0) + 1;
            else if (user.age < 35)
                ageCounts['25-34'] = (ageCounts['25-34'] || 0) + 1;
            else if (user.age < 45)
                ageCounts['35-44'] = (ageCounts['35-44'] || 0) + 1;
            else if (user.age < 55)
                ageCounts['45-54'] = (ageCounts['45-54'] || 0) + 1;
            else
                ageCounts['55+'] = (ageCounts['55+'] || 0) + 1;
        }
    });
    return {
        totalUsers,
...
function analyzeRidePatterns(trips) {
    let totalRiders = 0;
    let totalDuration = 0;
    let validDurationCount = 0;
    const riderCountDistribution = {};
    trips.forEach(trip => {
